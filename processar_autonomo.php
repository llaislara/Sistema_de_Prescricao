...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['prescricao']) && is_array($_POST['prescricao'])) {
        try {
            ...
            echo "<script>window.location = 'autonomo.php';</script>";
        } catch (Exception $e) {
            // Em caso de erro, exiba uma mensagem de erro
            echo 'Erro: ' . $e->getMessage();
        }
    } else {
        // Se 'prescricao' não é um array válido, redirecione de volta para a página de painel
        header('Location: autonomo.php');
        exit();
    }
} else {
    // Se o formulário não foi enviado corretamente, redirecione de volta para a página de painel
    header('Location: autonomo.php');
    exit();
}
?>
